 ...
 3. 비밀번호가 맞는 경우
   ...
   3. deleteForm.jsp 출력하기   
    ...
    public String delete(HttpServletRequest request, HttpServletResponse reponse) {
    	...
    	String pass = request.getParameter("pass");
    	MemberDao dao = new MemberDao();
    	if (login == null) { //로그아웃 상태 (로그인 여)
    		request.setAttribute("msg", "로그인 하세요");
    		request.setAttribute("url", "loginForm");
    		return "/view/alert.jsp";
    	} else if (!login.equals("admin") && !id.equals(login)) { // 본인 여부 확인 
    		request.setAttribute("msg", "본인 정보만 수정 가능합니다.");
    		request.setAttribute("url", "main");
    		return "/view/alert.jsp";
    	}
        ...
        if (id.equals("admin")) {
    		request.setAttribute("msg","관리자인 경우 탈퇴가 불가능합니다.");
    		return "/view/member/list.jsp";
    	}
    	else if(login.equals("admin")) { // 관리자인 경우
    		
    		Member mem = dao.selectOne("admin");
    		if(mem.getPass().equals(pass)) { // 비밀번호 일치 
    			dao.delete(id);
